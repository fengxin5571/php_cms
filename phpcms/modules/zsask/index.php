...
class index {
	...
	protected  $question, $answer, $comment, $zsask, $hits, $userid, $admin, $config, $IP,$udb,$memberinfo;
	...
	function __construct() {
		...
		$this->udb = pc_base::load_model('member_model');
		$this->memberinfo = $this->udb->get_one(array('userid'=>$this->userid));
		$this->IP = ip();
	}
	...
	function question() {
		...
		if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
			...
			$insert['catid']    = intval($_POST['catid']) ? intval($_POST['catid']) : 0;
	...
	function agreeAnswer() {
		...
		$rs = $this->question->get_one(array('qid'=>$qid, 'status' =>0));
		if (!$rs) showmessage('问题状态错误！');
		if($this->userid !=$rs[userid])showmessage('对不起! 您不是此问题的提问者');
